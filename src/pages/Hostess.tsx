...
export default function Hostess({ onRegistrarIngreso }: HostessProps) {
    ...
    const alEscanearExito = async (textoDecodificado: string) => {
      try {
        ...
        const { data: dataIntegrantes } = await supabase
          .from('integrantes')
          .select('*')
          .eq('family_id', idFamilia) // Ajustado temporalmente si tu campo usa family_id o familia_id
          .order('id', { ascending: true });

        if (dataFamilia) {
          setFamiliaActual(dataFamilia);
  ...
  const ejecutarCheckIn = async () => {
    ...
    setIntegrantesActuales(prev => prev.map(int => 
      seleccionados.includes(int.id) ? { ...int, asistio: true, hora_ingreso: ahora } : int
    ));
    
    // 🍞 DETONAR TOAST PREMIUM EN LUGAR DE UN ALERT FEO
    setToastMensaje(`¡Acceso Concedido! Bienvenidos Familia ${familiaActual?.nombre_familia}`);
    setSeleccionados([]);
  };

  const confirmados = integrantesActuales.filter(i => i.confirmado_individual);
      ...
      <div className="tarjeta-gala" style={{ maxWidth: '420px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        {modoEscaneo && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 className="titulo-gala" style={{ fontSize: '32px', margin: '10px 0 20px 0' }}>Escáner QR</h2>
        ...
        {!modoEscaneo && !loadingBusqueda && familiaActual && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '25px' }}>
              <span className="subtitulo-caps" style={{ fontSize: '11px' }}>Pase Detectado con Éxito</span>
              <h2 className="titulo-gala" style={{ fontSize: '32px', margin: '10px 0 5px 0' }}>Recepción</h2>
                    ...
                    >
                      <label style={{ 
                        ...
                        <input
                          ...
                          style={{ marginRight: '12px', transform: 'scale(1.2)', accentColor: '#7b1fa2' }}
                        />
                        <div>
                          <strong style={{ fontWeight: yaIngreso ? '500' : '600' }}>{persona.nombre}</strong>
                          {yaIngreso && (
